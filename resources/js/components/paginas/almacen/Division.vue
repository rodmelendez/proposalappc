<template>
    ...
    >
        <template slot="contenido_item" slot-scope="row">
            <h4><strong>{{ nombreBodega(row.item.id_bodega) }}</strong> - {{ row.item.nombre }}</h4>
        </template>
...
<script>
    export default {
        ...
        data: () => ({
            ...
            titulo_singular: 'División de bodega',
            titulo_plural: 'Divisiones de bodega',
            icono: 'icon-line-awesome-map-signs',
            items: [],
            item: {
                id: 0,
                id_bodega: 0,

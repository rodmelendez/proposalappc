...
<head>
    ...
    <style type="text/css">
        ...
        .avatar-usuario {
            ...
            overflow: hidden;
            margin: 0 auto;
            background: transparent url("{!! asset('img/default-avatar.jpg') !!}") no-repeat center center;
    ...
    <div class="login-cont">
        <div class="login">
            ...
            <!--div class="login__check">
                ...
                {{--<img src="{!! asset('img/logo.png') !!}" width="150" height="150" alt="INTRANET">--}}
            </div-->
            <figure class="avatar-usuario"></figure>
...
<script type="text/javascript">
    function enviarFormulario($frm, fn_done) {
        $.ajax({
            type: 'POST',
            url: $frm.attr('action'),
            dataType: 'json',
    ...
    function cargarAvatar(nombre_usuario) {
        const $cont = $('.login-cont');
        const $frm = $cont.find('#frm_login');

        if (typeof nombre_usuario === 'undefined') {
            nombre_usuario = $frm.find('input.name').val();
        }

        $.ajax({
            type: 'GET',
            url: '{!! route('cargar_avatar_usuario') !!}',
            dataType: 'json',
            data: {
                _fuente: 'User',
                _accion: 'cargarAvatar',
            ...
            if (data['ok']) {
                if (typeof data['avatar'] === 'string' && data['avatar'].length) {
                    $cont
                        .find('.avatar-usuario')
                        .css('backgroundImage', 'url(' + data['avatar'] + ')');

                    guardarEnSesion('nombre_usuario', nombre_usuario);
                    guardarEnSesion('avatar_usuario', data['avatar']);
                }

                //color de fondo del formulario
                {
                    const $login = $cont.find('.login');
                    if (typeof data['color_fondo'] === 'string' && data['color_fondo']) {

                        const color_fondo = data['color_fondo'];
                        establecerColorFondo(color_fondo, $login);

                        guardarEnSesion('fonto_color_usuario', color_fondo);
                    }
                    else {
                        //$login.css('backgroundImage', '');
    ...
    function establecerColorFondo(color_fondo, $login) {
        if (typeof $login === 'undefined') {
            $login = $('.login');
        }
        const color_fondo_darker = variarColor(-0.9, '#' + color_fondo);
        $login.attr('style', `background-image:linear-gradient(top, #${color_fondo}F2 0%, ${color_fondo_darker}E6 100%) !important; background-image:-webkit-linear-gradient(top, #${color_fondo}F2 0%, ${color_fondo_darker}E6 100%) !important;`);
        /*background-image: linear-gradient(top, #${color_fondo}F2 0%, #${color_fondo}E6 100%) !important;
        background-image: -webkit-linear-gradient(top, #${color_fondo}F2 0%, #${$color_fondo}E6 100%) !important;*/
    }
    ...
    function fondoSinConexion() {
        if (typeof navigator === 'object' && !navigator.onLine) {
            const $contenedor = $('.cont');
            $contenedor.css('background-image', 'url({!! asset('img/bg.jpg') !!})');
        }
    }


    function cargarUsuarioDeSesionLocal() {
        const $contenedor = $('.cont');
        const nombre_usuario = traerDesdeSesion('nombre_usuario');

        if (typeof nombre_usuario === 'string' && nombre_usuario.length) {
            $contenedor.find('.login__input.name').val(nombre_usuario);

            const avatar_usuario = traerDesdeSesion('avatar_usuario');
            if (typeof avatar_usuario === 'string' && avatar_usuario.length) {
                $contenedor.find('.avatar-usuario').css('backgroundImage', 'url(' + avatar_usuario + ')');
            }

            const fonto_color_usuario = traerDesdeSesion('fonto_color_usuario');
            if (typeof fonto_color_usuario === 'string' && fonto_color_usuario.length) {
                establecerColorFondo(fonto_color_usuario);
            }

            $contenedor.find('.login__input.pass').focus();
        }
    }


    function cancelarCargaDeSesionLocal() {
        const $contenedor = $('.cont');

        $contenedor.find('.avatar-usuario').css('backgroundImage', '');
        $contenedor.find('.login__input.pass').val('');
        $contenedor.find('.login__input.name').val('').focus();
    }
    ...
    function atarAcciones() {
        ...
        $frm.find('input.name').keypress(function(e) {
            ...
            cargarAvatar(this.value);
        });

        //Si se presiona Esc en el campo contraseña, se quitan los valores precargados desde la sesión lcoal
        $frm.find('input.pass').keydown(function(e) {
            if (e.key === 'Escape') {
                cancelarCargaDeSesionLocal();
            }
        });
